/*
   Description : code to count frequency and calculate fft 
*/
...
#define N 8000  		           		// Specifies number of complex FFT points
#define THRESH 0.5
...
void main()
{
  ...
  while (1) 							//infinite loop
  {
	...
	x[i].im = 0.0 ;				  		  //zero imaginary part
        ...
        x1[i].re = (float)((short)input_right_sample());
        x1[i].im = 0.0
       
        
        if ( (x1[i] > thresh) && (x1[i-1] < thresh) ) 
           count ++ ;                                         // so whenever it crosses threshold, iut counts 
